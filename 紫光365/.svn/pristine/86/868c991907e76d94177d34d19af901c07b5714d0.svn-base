...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        [self config:Dragging_upRefresh];
    }
...
- (void) config:(Dragging_type)type
{
    ...
    //上啦加载更多
    if (type == Dragging_upLoadmore) {
        refreshView.type = YES;
        loadMoreView.type = YES;
    }else{
        refreshView.type = NO;
        loadMoreView.type = NO;
    }
    /**///new
     self.pullBackgroundColor = [UIColor clearColor];
    self.pullArrowImage = [UIImage imageNamed:@""];
}
