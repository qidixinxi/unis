...
//    ChatRecorderView        *recorderView;      //录音界面
    ChatRecordVoiceView        *recorderView;      //录音界面
...
- (void)initRecordView{
    ...
    */
    ...
    if (recorderView == nil)
        recorderView = [[ChatRecordVoiceView alloc] initWithFrame:CGRectZero];
    //还原界面显示
    [recorderView restoreDisplay];
    NSLog(@"======");
}
...
- (void)removeScreenTouchObserver{
    ...
    NSLog(@"并为执行---removeScreenTouchObserver");
}
#pragma mark - 添加触摸观察者
- (void)addScreenTouchObserver{
    ...
    NSLog(@"执行监听---addScreenTouchObserver");
}
// 触摸观察者模式
...
//    CGFloat length = timer.timeInterval;//自己添加的
    //停止计时器
    [self stopTimer];
    ...
    [UIView hideViewByCompletion:^(BOOL finish){
        ...
        if (canNotSend) {
            ...
            */
            //自己添加的
            //
            if ([self.vrbDelegate respondsToSelector:@selector(VoiceRecorderBaseVCRecordFinish:fileName:withVoiceLenth:)]) {
                [self.vrbDelegate VoiceRecorderBaseVCRecordFinish:recordFilePath fileName:recordFileName withVoiceLenth:curCount];
            }
        }
