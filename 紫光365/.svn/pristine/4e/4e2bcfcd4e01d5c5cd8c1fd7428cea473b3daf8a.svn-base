...
//	[delegate performSelector:@selector(timeoutCallBack:) withObject:self];
    ...
    /*罗修改数据*/
    if (self.delegate &&[self.delegate respondsToSelector:@selector(timeoutCallBack:)]) {
        NSLog(@"asiformdata 超时回调 数据 很严重");
        [delegate performSelector:@selector(timeoutCallBack:) withObject:self];
    }
}
/*******/
-(void)timeoutCallBack:(id)request{
    NSLog(@"---->超时回调");
}
/*******/
...
- (void)addPostValue:(id <NSObject>)value forKey:(NSString *)key
{
    ...
    NSLog(@"key:%@   value:%@",key,[value description]);
