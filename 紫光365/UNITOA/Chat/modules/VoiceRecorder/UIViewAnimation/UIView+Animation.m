...
//    return [[[self getAppDelegate] viewController_] view];
    
//    return [[[UIApplication sharedApplication] delegate] window];
    return [[[[UIView getAppDelegate] window] rootViewController] view];
//    return [[[UIView getAppDelegate] viewController_] view];
//    [[[UIViewController alloc] init] view];
}
...
- (void)showInview:(UIView *)_view animateType:(AnimateType)_aType{
    //
    if (!self.superview) {
        [_view addSubview:self];
    }
    self.alpha = 1;
//    if (AnimateTypeOfStartOrgFold_l) {
...
//        return;
//    }
    if (_aType == AnimateTypeOfFold_l || _aType == AnimateTypeOfStartOrgFold_l) {
        self.layer.transform = CATransform3DMakeScale(1.0, 0.01, 1.0);
...
- (void)hideAnimateType:(AnimateType)_aType{
    if (_aType == AnimateTypeOfFold_l || _aType == AnimateTypeOfStartOrgFold_l) {
        self.layer.transform = CATransform3DIdentity;
        [UIView animateWithDuration:.3 animations:^{
            self.layer.transform = CATransform3DMakeScale(1.0, 0.01, 1.0);
