//
//  CreateSginListViewController.m
//  UNITOA
//
...
@interface CreateSginListViewController ()<UITableViewDataSource,UITableViewDelegate,PullTableViewDelegate,RefrashDelegate>
{
...
@end
@implementation CreateSginListViewController
...
- (void)getSigninList{
    ...
    [AFRequestService responseData:FACE_SIGNIN_LIST andparameters:parameter andResponseData:^(NSData *responseData) {
            ...
            if([signinlist isKindOfClass:[NSArray class]]){
                for (int i = 0; i<[signinlist count];i++ ) {
                    ...
                    if ([signIfo isKindOfClass:[NSDictionary class]]) {
                        ...
                        signModel.userCount = [signIfo objectForKey:@"userCount"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.expireDate.text = [NSString stringWithFormat:@"过期时间：%@",[[model.expireDate componentsSeparatedByString:@" "] firstObject]];//[[model.expireDate componentsSeparatedByString:@" "] firstObject];
...
{
    SinIn_NewModel * model = (SinIn_NewModel *)newSignList[indexPath.row];
    MyNewSignViewController *newSign = [MyNewSignViewController new];
    newSign.delegate = self;
    newSign.signModel = model;
    [self.navigationController pushViewController:newSign animated:YES];
}
- (void)refreshData{
    [ZSTool presentAlert:@"签到作废成功"];
    currentPage = 1;
    [self getSigninList];
}
