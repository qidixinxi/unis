...
- (void)layoutSubviews
{
    ...
    if ([_articleModel.isShare isEqualToString:ISNOT_SHARE_CODE]) {
        ...
        if (_articleModel.fromWeixin.length == 0 && temp.length >0) {
            // 是分享的链接
            self.shareComment.frame = CGRectZero;
            self.shareContext.frame = CGRectMake(0, 0, 250,[SingleInstance customFontHeightFont:_articleModel.context andFontSize:14 andLineWidth:250] + 5);
            ...
            self.shareContext.frame = CGRectMake(0, self.shareComment.frame.origin.y + self.shareComment.frame.size.height + 5, 250,[SingleInstance customFontHeightFont:_articleModel.context andFontSize:14 andLineWidth:250] + 5 );
            self.tempView.frame = CGRectMake(0, self.shareContext.frame.origin.y - 1, SCREEN_WIDTH, 3);
            self.contact.frame = CGRectMake(USER_ICON_WHDTH + 15, 5, 250,self.shareContext.frame.size.height + self.shareContext.frame.origin.y);
        }
...
+ (CGFloat)heightForCellWithPost:(UserArticleList *)articleModel
{
    ...
    if ([articleModel.isShare isEqualToString:ISNOT_SHARE_CODE]) {
        ...
        // 如果不含图片在内
        if ([articleModel.photo isEqualToString:@""]||articleModel.photo == nil || articleModel.photo.length == 0) {
            imgHeight = 0;
        }
        // 包含图片
        else{
        ...
        if (articleModel.fromWeixin.length == 0 && temp.length >0) {
            ...
            // 发表的内容
           contentHeight = [SingleInstance customFontHeightFont:articleModel.context andFontSize:14 andLineWidth:250]+10 + [SingleInstance customFontHeightFont:articleModel.shareComment andFontSize:14 andLineWidth:250];
        }
    }
