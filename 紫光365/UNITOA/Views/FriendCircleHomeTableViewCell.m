...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        // 多图的显示
        self.PictureViews = [[ZSCircleImageView alloc] initWithFrame:CGRectMake(60,0,240,0)];
...
- (void)delOwenComment:(id)commentId andCell:(id)cell{
    [self.delegate deleteOwnComment:commentId andCell:cell andHomeCell:self];
}
- (void)setPost:(UserArticleList *)post
{
    ...
    // 用户的头像
    [self.userIcon sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",IMAGE_BASE_URL,_post.iconUrl]] placeholderImage:[UIImage imageNamed:@"portrait_ico"]];
...
- (void)layoutSubviews{
    ...
    CGRect rectr = [SingleInstance getFrame:self.userName.text andWidth:SCREEN_WIDTH - 30];
    self.userName.frame = CGRectMake(USER_ICON_WHDTH + 20, 25, rectr.size.width, USER_NAME_HEIGHT);
    self.urlLabel.frame = CGRectMake(self.userName.frame.size.width + self.userName.frame.origin.x + 5, 26, URL_LABEL_WHDTH, URL_LABEL_HEIGHT);
    
    // 内容的高度
    CGFloat contentHeight = [FriendContentView heightForCellWithPost:_post];
    _ContentView.frame = CGRectMake(0,50, SCREEN_WIDTH,contentHeight);
    ...
    if (_post.photo.length > 0 && ![_post.photo isKindOfClass:[NSNull class]])
    {
        ...
        [_single_imageView sd_setImageWithURL:[ZSTool returnUrl:_post.photo] placeholderImage:[UIImage imageNamed:@"guke_image_loading"]];
    }
    // 单张的图片不存在
    else
    {
...
{
    CGFloat shareHeight = 0.0;//赞的高度
    CGFloat commentHeight = 0.0;// 评论的高度
    CGFloat content_height = 0.0;// 图片的高度
    if (post.photo.length > 0 && ![post.photo isKindOfClass:[NSNull class]] && post.photo.length != 0)
    {
        content_height = 0;
    }
    // 单张的图片不存在
    ...
    {
        // 存有多个附件
        if (post.attachlistArray.count)
        {
            int i = post.attachlistArray.count/3;
            
            int j = post.attachlistArray.count%3?1:0;
            
            content_height = 75*(i+j)+2.5*(j + i - 1);
    }
 }
    // 赞的高度
    if ([post.goodArray count]>0) {
        shareHeight = [SingleInstance customHeight:[post.goodArray count] andcount:5 andsingleHeight:35.0] +7;
    }
    // 评论的高度设定
    if ([post.commentArray count]>0) {
        ...
        commentHeight = commentHeight + [post.commentArray count] *4 + 10;
    }
    else{
        commentHeight = 0.0;
    }
    
   CGFloat height = 80 + [FriendContentView heightForCellWithPost:post] + REPORT_TIME_HEIGHT + shareHeight + commentHeight +content_height;
    return height;
}
